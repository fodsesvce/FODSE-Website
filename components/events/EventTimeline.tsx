...
className="group relative h-[620px] rounded-3xl overflow-hidden bg-surface border border-border transition-all duration-500 cursor-pointer shadow-md hover:-translate-y-2 hover:shadow-[0_18px_40px_rgba(37,99,235,0.18)] hover:border-accent"
              >
                {/* 1. Header Visual (Representing Event Image) */}
                {/* Header */}

<div className="relative h-[420px] w-full overflow-hidden border-b border-border bg-black">
  ...
  {event.image ? (
    ...
    alt={event.title}
    fill
    className="object-contain bg-black transition-transform duration-500 group-hover:scale-[1.03]"
/>
    ...
    <div
      className={`relative h-full w-full bg-gradient-to-br ${gradient} flex items-center justify-center p-6`}
    >
      ...
      <div className="absolute w-32 h-32 rounded-full bg-white/10 blur-xl animate-pulse" />

    </div>

  )}

  <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md border border-white/20 text-white text-[10px] font-bold px-3 py-1 rounded-full">
...
>
    {event.title}
                ...
                <div className="p-6 flex flex-col justify-between h-[200px] bg-surface transition-colors duration-300">
